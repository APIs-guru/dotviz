...
typedef struct {
  ...
  pointf (*gen)(output_string *output, obj_state_t *obj, pointf p, pointf u,
                double arrowsize, double penwidth,
                uint32_t flag); ///< generator function for
                                ///< type
...
/* arrowOrthoClip:
 * For orthogonal routing, we know each Bezier of spl is a horizontal or
...
/* Draw a concave semicircle using a single cubic bezier curve that touches p at
 * its midpoint. See
