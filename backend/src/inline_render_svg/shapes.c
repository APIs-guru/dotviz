...
static polygon_t p_diamond = {
    .peripheries = 1, .sides = 4, .orientation = 45.0};
static polygon_t p_trapezium = {
    .peripheries = 1, .sides = 4, .distortion = -0.4};
...
static bool IS_BOX(node_t *n) { return ND_shape(n)->polygon == &p_box; }

static bool IS_PLAIN(node_t *n) { return ND_shape(n)->polygon == &p_plain; }
...
static shape_functions poly_fns = {poly_init,   poly_free, poly_port,
                                   poly_inside, poly_path, poly_gencode};
static shape_functions point_fns = {point_init,   poly_free, poly_port,
                                    point_inside, NULL,      point_gencode};
static shape_functions record_fns = {record_init, record_free,
                                     record_port, record_inside,
                                     record_path, record_gencode};
static shape_functions epsf_fns = {epsf_init,   epsf_free, poly_port,
                                   epsf_inside, NULL,      epsf_gencode};
static shape_functions star_fns = {poly_init,   poly_free, poly_port,
                                   star_inside, poly_path, poly_gencode};
static shape_functions cylinder_fns = {poly_init,   poly_free, poly_port,
                                       poly_inside, poly_path, poly_gencode};

static shape_desc Shapes[] =
    {/* first entry is default for no such shape */
...
static void rounded_draw(GVJ_t *job, pointf *AF, size_t sides,
  ...
  pts[i++] = pts[0];
  pts[i++] = pts[1];
  svg_bezier(job, pts + 1, i - 1, filled);
  free(pts);
  free(B);
}
...
void round_corners(GVJ_t *job, pointf *AF, size_t sides,
  ...
  case DOGEAR:
    ...
    svg_polygon(job, D, sides + 1, filled);
    free(D);
  ...
  case TAB:
    ...
    for (size_t seg = 4; seg < sides + 2; seg++)
      D[seg] = AF[seg - 2];
    svg_polygon(job, D, sides + 2, filled);
    free(D);
  ...
  case FOLDER:
    ...
    for (size_t seg = 4; seg < sides + 3; seg++)
      D[seg] = AF[seg - 3];
    svg_polygon(job, D, sides + 3, filled);
    free(D);
    break;
  case BOX3D:
    assert(sides == 4);
  ...
  case COMPONENT:
    ...
    D[10] = AF[2];
    D[11] = AF[3];
    svg_polygon(job, D, sides + 8, filled);
  ...
  case PROMOTER:
    ...
    svg_polygon(job, D, sides + 5, filled);
  ...
  case TERMINATOR:
    ...
    D[4].x = mid_x(AF) -
             (B[2].x - B[3].x) * 3 / 4; // D[3].y mirrored across the center
    D[4].y = D[3].y;
    D[5].x = D[4].x;
    D[5].y = D[2].y;
    D[6].x =
        mid_x(AF) - (B[2].x - B[3].x) / 4; // D[1].x mirrored across the center
    D[6].y = D[1].y;
    D[7].x = D[6].x;
    D[7].y = D[0].y;
    svg_polygon(job, D, sides + 4, filled);
  ...
  case UTR:
    ...
    D[3].x =
        mid_x(AF) - (B[2].x - B[3].x) / 4; // D[2].x mirrored across the center
    D[3].y = D[2].y;
  ...
  case PRIMERSITE:
    ...
    D[3].x =
        mid_x(AF) - (AF[0].x - AF[1].x) / 4; // x_center - 2*(scalable width)
    D[3].y = D[2].y;
    D[4].x = D[3].x;
    D[4].y = D[0].y;
    svg_polygon(job, D, sides + 1, filled);
  ...
  case FIVEPOVERHANG:
    ...
    D[3].x = D[0].x;
    D[3].y = D[2].y;
    svg_polygon(job, D, sides, filled);
  ...
  case NOVERHANG:
    ...
    D[0].x =
        mid_x(AF) - (B[2].x - B[3].x) * 9 / 8; // x_center - 2*width - 1/4*width
    D[0].y = mid_y(&AF[1]) -
             (B[3].y - B[4].y) * 5 / 8; // y_center - width - 1/4 width
    D[1].x = D[0].x + (B[2].x - B[3].x);
    D[1].y = D[0].y;
    D[2].x = D[1].x;
    D[2].y = D[1].y + (B[3].y - B[4].y) / 2;
    ...
    D[0].x = mid_x(AF) + (B[2].x - B[3].x) / 8; // x_center + 1/4*width
    D[0].y =
        mid_y(&AF[1]) + (B[3].y - B[4].y) / 8; // y_center - width - 1/4 width
    D[1].x = D[0].x + (B[2].x - B[3].x);
    D[1].y = D[0].y;
    D[2].x = D[1].x;
    D[2].y = D[1].y + (B[3].y - B[4].y) / 2;
    ...
    /*dsDNA line left half*/
    C[0].x = mid_x(AF) -
             (B[2].x - B[3].x) * 9 / 8; // D[0].x of of the left rectangles
    C[0].y = mid_y(&AF[1]);
    C[1].x = AF[1].x;
    C[1].y = AF[2].y + (AF[0].y - AF[3].y) / 2;
    svg_polyline(job, C, 2);
  ...
  case RIBOSITE:
    ...
    D[2].x =
        D[0].x - (B[2].x - B[3].x) / 8; // D[0].x- width/4 //right nook of the x
    ...
    D[15].x = D[2].x;
    D[15].y = D[0].y;
    svg_polygon(job, D, sides + 12, filled);
...
static void poly_init(node_t *n) {
  ...
  if (ND_shape(n)->usershape) {
    ...
    if (streq(ND_shape(n)->name, "custom")) {
      ...
      if (imagesize.x == -1 && imagesize.y == -1) {
        agwarningf("No or improper shapefile=\"%s\" for node \"%s\"\n",
    ...
    imagesize = gvusershape_size(agraphof(n), sfile);
    if (imagesize.x == -1 && imagesize.y == -1) {
      agwarningf("No or improper image=\"%s\" for node \"%s\"\n",
                 sfile ? sfile : "<nil>", agnameof(n));
      imagesize.x = imagesize.y = 0;
    } else {
      GD_has_images(agraphof(n)) = true;
  ...
  if (sides <= 2 && (!is_exactly_zero(distortion) || !is_exactly_zero(skew))) {
    sides = 120;
  }
  ...
  if (*fxd == 's' && streq(fxd, "shape")) {
    ...
    if (width < ND_label(n)->dimen.x || height < ND_label(n)->dimen.y)
      agwarningf("node '%s', graph '%s' size too small for label\n",
  ...
  if (sides < 3) { /* ellipses */
    ...
    if (ND_shape(n)->polygon->vertices) {
      ...
      for (size_t i = 0; i < sides; i++) {
        ...
        /*distort and skew */
        pointf P = {.x = R.x * (skewdist + R.y * gdistortion) + R.y * gskew,
                    .y = R.y};
...
static bool poly_inside(inside_t *inside_context, pointf p) {
  ...
  if (n != inside_context->s.lastn) {
    ...
    if (inside_context->s.last_poly->peripheries >= 1 && penwidth > 0) {
      /* index to outline, i.e., the outer-periphery with penwidth taken into
       * account */
  ...
  for (size_t j = 1; j < sides; j++) { // iterate over remaining segments
    ...
    if (!same_side(
            P, O, vertex[i + inside_context->s.outp],
            vertex[i1 + inside_context->s.outp])) { // false if outside any
                                                    // other segment’s face
    ...
    case '\000':
      ...
      if (mode & HASTEXT) {
        if (tsp > text + 1 && tsp - 1 != hstsp && *(tsp - 1) == ' ')
          tsp--;
        *tsp = '\000';
        fp->lp = make_label(n, text, lbl->html ? LT_HTML : LT_NONE,
...
static void record_init(node_t *n) {
  ...
  if (mapbool(late_string(n, N_fixed, "false"))) {
    if (sz.x < info->size.x || sz.y < info->size.y) {
      /* should check that the record really won't fit, e.g., there may be no
         text. agwarningf("node '%s' size may be too small\n", agnameof(n));
      */
    }
  } else {
  ...
  ND_height(n) =
      PS2INCH(info->size.y +
              1); /* Kluge!!  +1 to fix rounding diff between layout and
                     rendering otherwise we can get -1 coords in output */
...
static void cylinder_draw(GVJ_t *job, pointf *AF, size_t sides, int filled) {
  ...
  svg_bezier(job, AF, sides, filled);
  svg_bezier(job, vertices, 7, 0);
}
