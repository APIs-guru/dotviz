    ...
    it('returns error messages for invalid options', async () => {
      ...
      expectFailureResult(result).toMatchInlineSnapshot(`
        [
          {
            "level": "error",
            "message": "JSON error UnexpectedToken at 1:335: \`lse,"engine":"dot","yInvert":"bad value","reduce":false,"images":{}}\`",
          },
        ]
      `);
    });
    ...
    it('returns error messages for layout errors', async () => {
      ...
      expectFailureResult(result).toMatchInlineSnapshot(`
        [
          {
            "level": "error",
            "message": "Layout type: "invalid" not recognized. Use one of: dot circo neato",
          },
        ]
      `);
    });
