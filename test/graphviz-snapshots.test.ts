...
import { type MultipleRenderResult as VizJSResult } from '@viz-js/viz';
...
describe('tests from GraphViz Gallery', () => {
  ...
  describe('sfdp', () => {
    it.skip('sfdp: gallery/sfdp/root.gv', snapshotGvFile);
  });
  describe('twopi', () => {
...
function compareWithVizJS(
  ...
  expect(dotvizResult.output?.dot).toStrictEqual(vizjsResult.output?.dot);
  const vizjsSvg = vizjsResult?.output?.svg.replaceAll(
    'Generated by graphviz version 13.1.2 (20250808.2320)',
    'Generated by graphviz version a (a)',
  );
  expect(dotvizResult.output?.svg).toStrictEqual(vizjsSvg);
}
...
async function snapshotGvFile({ task }: TestContext) {
  ...
  const dotvizResult = dotviz.renderDot(gvString, {
    formats: ['dot', 'svg'],
    engine,
    overrideAttributes,
  });
  ...
  const vizjsResult = vizjs.renderFormats(gvString, ['dot', 'svg'], {
    ...overrideAttributes,
    engine,
  });
