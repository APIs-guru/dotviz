...
async function snapshotGvFile({ task }: TestContext) {
  ...
  if (USE_VIZ_JS) {
    ...
    await expectString(output.dot).toMatchFileSnapshot(dotSnapshotPath);
    const svg = output.svg.replaceAll(
    ...
    expect(filterErrors(diagnostics)).toStrictEqual([]);
    assert.exists(output);

    await expectString(output.dot).toMatchFileSnapshot(dotSnapshotPath);
    await expectString(output.svg).toMatchFileSnapshot(svgSnapshotPath);
  }
}
